...
module "lb_cluster" {
  ...
  // Configuration LB
  configuration_lb = {
    name               = "network-${lookup(element(var.ecs_fargate, count.index), "ecr_repository")}"
...
locals {
  ecs_nlb_env_vars = {
    for i, lb_output in module.lb_cluster :
    "NLB_DNS_NAME_${upper(replace(element(var.ecs_fargate, i).ecr_repository, "-", "_"))}" => lb_output.lb_reference.dns_name
  }
}

# resource "aws_api_gateway_vpc_link" "api_gateway_vpc_link" {
#   for_each = locals.nlb_outputs_map

#   name = replace(
#     "vpclink-${lower(each.key)}-${var.layer}-${var.stack_id}",
#     "_",
#     "-"
#   )

#   target_arns = [each.value.lb_reference.arn]

#   tags = merge(var.tags, {
#     Name        = "vpclink-${lower(each.key)}-${var.layer}-${var.stack_id}"
...
resource "aws_lb_listener" "lb_listener" {
  ...
  load_balancer_arn = module.lb_cluster[count.index].lb_reference.id
  port              = lookup(element(var.ecs_fargate, count.index), "port")
  protocol          = "TCP"
